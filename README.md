...
# Homebridge GivEnergy Inverter Plugin

</span>

The initial version of this plugin is to allow automations to be created based on
the flow of energy and the state of the battery charge. The System is represented by a switch (on when PV is generating, 
off when no generation is happening) light level sensors to represent the PV Generation, Grid Import/export.

Clicking the switch currently performs no action, as the data is updated every 60 seconds
the switch will toggle back to reflect the current state of the Solar Array.
...
Login to the GivEnergy Cloud portal and generate an [API Token](https://givenergy.cloud/account-settings/api-tokens)

### TODO

- Validate multiple inverters are handled
- Control charge/discharge of battery
- Add test coverage
